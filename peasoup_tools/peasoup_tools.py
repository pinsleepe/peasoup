...
class OverviewFile(object):
    ...
    def make_predictor(self, idx):
        ...
        # from V.Morello
        def convert_period(period_peasoup, accel, nsamp, tsamp):
            """
                period_peasoup: topocentric period returned by peasoup (seconds),
                    measured at the mid-point of the observation segment processed,
                    which should be the first 2^n samples, with n as large as possible.
                accel: acceleration (m/s^2)
                nsamp: number of samples in the observation. Can be read from peasoup's output XML.
                tsamp: sampling time in seconds. Can be read from peasoup's output XML.
            """
            clight = 299792458.0

            # peasoup uses only a power-of-two number of samples
            nsamp = 2 ** int(log2(nsamp))
            tobs_peasoup = nsamp * tsamp

            # Period at the start of the observation (what dspsr wants)
            period_start = (1.0 - accel / clight * tobs_peasoup / 2.0) * period_peasoup
            return period_start

        ra = radec_to_str(float(header.find("src_raj").text))
        dec = radec_to_str(float(header.find("src_dej").text))
        ns = float(header.find("nsamples").text)
        ts = float(header.find("tsamp").text)

        new_period = convert_period(cand["period"], cand["acc"], ns, ts)
...
#         for ii, harm in enumerate(harms):
#             subcand = cand[np.where(cand["nh"] == harm)]
#             ax.scatter(subcand["dm"], subcand["snr"],
#                        facecolor=colors[ii],
#                        edgecolor="none",
#                        label="Harm. %d" % harm)
